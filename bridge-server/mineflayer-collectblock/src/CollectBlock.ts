    ...
    switch (closest.constructor.name) {
      case "Block": {
        try {
          ...
          await bot.tool.equipForBlock(closest as Block, equipToolOptions);
      ...
      case "Entity": {
        ...
        try {
          ...
          const waitForPickup = new Promise<void>((resolve, reject) => {
          ...
          bot.pathfinder.setGoal(new goals.GoalFollow(closest as Entity, 0));
...
async function mineBlock(
  ...
  tempEvents.subscribeTo("itemDrop", (entity: Entity) => {
    if (
      entity.position.distanceTo(block.position.offset(0.5, 0.5, 0.5)) <= 0.5
    ) {
