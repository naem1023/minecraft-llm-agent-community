...
export async function emptyInventoryIfFull(
    ...
    // @ts-expect-error
    if (cb != null)
        return callbackify(emptyInventoryIfFull)(bot, chestLocations, cb);
...
async function tryEmptyInventory(
    ...
    chestLocation: Vec3,
    itemFilter: ItemFilter,
    cb?: (err: Error | undefined, hasRemaining: boolean) => void,
): Promise<boolean> {
    // @ts-expect-error
    if (cb != null)
        return callbackify(tryEmptyInventory)(
            bot,
            chestLocation,
            itemFilter,
            cb,
...
async function placeItems(
    ...
    // @ts-expect-error
    if (cb != null)
        return callbackify(placeItems)(bot, chestPos, itemFilter, cb);
